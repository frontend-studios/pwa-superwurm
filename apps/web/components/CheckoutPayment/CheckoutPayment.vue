    ...
    <div v-if="paymentMethods?.list && paymentMethods.list.length > 0" class="grid gap-4 grid-cols-2">
      <label v-for="paymentMethod in paymentMethods.list" :key="paymentMethod.id" class="relative">
        ...
        <span
          ...
          class="peer-focus:border-primary-50 peer-focus:bg-primary-50 h-20 flex flex-col items-center justify-center py-4 px-4 cursor-pointer rounded-md border border-secondary-500 -outline-offset-2 hover:border-primary-50 active:border-primary-100 active:bg-primary-50 peer-checked:outline peer-checked:outline-2 peer-checked:outline-primary-500 peer-disabled:opacity-50 peer-disabled:bg-neutral-100 peer-disabled:border-neutral-200 peer-disabled:cursor-not-allowed peer-disabled:[&_img]:grayscale"
        >
